...
def get_args():
    ...
    parser.add_argument('--seed', type=int, default=None, help='set None to disable')
    ...
    ## pretrained model
    parser.add_argument("--weight_path", default="D:\\CrissCross\\weights\\aud_crisscross_kinetics_400.pth.tar", help="checkpoint_path for backbone restoration. linux: vid_random_tc/34474_8x112")

    ## dataset and config
    parser.add_argument("--db", default="esc50", help="target db", choices=['esc50', 'dcase'])
    parser.add_argument('-c', '--config-file', type=str, help="pass config file name w/o extension", default="config_fold1_5s")
...
def main_worker(gpu, ngpus_per_node, args, cfg):
    ...
    logger, tb_writter, wandb_writter = environ.prep_environment(args, cfg)
    ...
    if os.path.isfile(path):
        ...
        raise FileNotFoundError (f'pretext file is not found at {path}')

    if args.db == 'esc50':
        from evaluate.audio.audio_svm import linear_svm as worker
        worker(args, cfg, state, logger=logger, tb_writter=tb_writter, wandb_writter=wandb_writter)
    elif args.db == 'dcase':
        from evaluate.audio.audio_fctune import fctune as worker
